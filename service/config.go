...
func (s *Service) Config(code *code.Code) error {
	...
	for i = range s.Cfg.Config {
		...
		case 1:
			code.MainWriteString("\"\", \"\", false, new(", cfgPackageName, ".", s.Cfg.Config[i].Name, "), &app.RegisterCenter{\n\t\tGroupId: \"", s.Cfg.Config[i].Nacos.GroupId, "\",\n\t\tDataId: \"", s.Cfg.Config[i].Nacos.DataId, "\",\n")
			//update hook
			...
			code.MainWriteString("\t}")
		default:
			code.MainWriteString("\"", s.Cfg.Config[i].Local.Name, "\", \"", s.Cfg.Config[i].Local.Path, "\", true, new(", cfgPackageName, ".", s.Cfg.Config[i].Name, "), nil")
		}
		code.MainWriteString("); err != nil {\n",
			"\t\tpanic(err)\n",
			"\t}\n",
		)
		if s.Cfg.Config[i].Hook != nil && len(s.Cfg.Config[i].Hook.Get) > 0 {
